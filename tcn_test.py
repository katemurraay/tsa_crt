    ...
    for t in targets:
        for c in cryptos:
            ...
            for index, r in enumerate(retrain):
                ...
                experiment_name = 'tcn-' + c + '-' + t  + '-w' + str(window) +  '-h' + str(h) + '-' + str(len(outputs)) +'m'
                ...
                if tuned:
                    parameters = pd.read_csv("param/p_TCN-"+t+ '-'+c +"-w30-h0.csv").iloc[0] 
                    p = {'conv_filter': parameters['conv_filter'],
                        'conv_kernel': parameters['conv_kernel'],
                        'conv_activation': parameters['conv_activation'],
                        'dropout_rate': parameters['dropout_rate'],
                        'dense_dim': parameters['dense_dim'],
                        'dilation_rate': parameters['dilation_rate'],
